...
class ActiveDirectoryUser
  ...
  # Strip out brackets and quotations from single valued attributes in AD
  first = Proc.new {|n| n.gsub(/[\[\]\"]/, '')}
  ...
  def self.authenticate(login, pass)
    ...
    conn = Net::LDAP.new :host => AD['SERVER'],
                         :port => AD['PORT'],
                         :base => AD['BASE'],
                         :auth => { :username => AD['LOGIN'],
                                    :password => AD['PASS'],
                                    :method => :simple }
    if result = conn.bind_as(:filter => "sAMAccountName=#{login}", :password => pass)
